...
The adapter calls the current backend session and recording routes from `src/lib/api.ts`. It submits only the ZIP and selected research privacy mode—never a patient reference or the original filename as a public value. `metadata-scrub` preserves waveform values while removing EDF metadata; `signal-obfuscation` additionally applies a keyed, lossy transformation before detection and privacy evaluation. Neither mode is an anonymity guarantee.

Waveform access is intentionally disabled. The results page renders a bounded
score timeline, threshold, and model-alert windows without downloading EEG
samples.
...
- `/results/[recordId]` - review one recording’s model-alert score timeline, dataset references, and separately labelled non-clinical development scores.
...
6. `src/components/ResultScreen.tsx`, `RecordingNavigator.tsx`, and `PredictionTimeline.tsx` - session-scoped result review and model score timeline;
