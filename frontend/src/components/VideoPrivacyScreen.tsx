    ...
    try {
      ...
      setError(
        submissionError instanceof Error
          ...
          : "The video could not be secured.",
      );
    }
  }
    ...
    <div className="page-frame">
      <div className="mx-auto max-w-4xl animate-enter-up">
        <div className="max-w-3xl">
          <h1 className="text-[clamp(2rem,5vw,2.25rem)] font-semibold leading-[1.08] tracking-[-0.03em] text-ink">
        ...
        <form
          className="mt-10 grid items-start gap-6 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]"
        ...
        >
          ...
          >
            ...
            <div className="px-5 py-6 sm:px-7">
              <label
                className="flex min-h-40 cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed border-rule-strong bg-surface-soft px-6 py-7 text-center outline-none transition-colors hover:border-teal hover:bg-teal-soft/40 focus-within:border-teal focus-within:ring-4 focus-within:ring-teal/20"
                htmlFor="video-file"
              >
                <Icon
                  name={submitting ? "spinner" : "file"}
              ...
              {file && (
                <p
                  className="mt-4 rounded-md bg-surface-soft px-3 py-2 text-xs leading-5 text-ink-muted"
                  aria-live="polite"
                >
                  Ready to secure {formatBytes(file.size)}. The generated job
                  label will be shown instead of this filename.
                </p>
              )}
              {submitting && (
                <div className="mt-5" aria-live="polite">
                  <div className="flex justify-between text-xs text-ink-muted">
                    <span>Securing video</span>
            ...
            <div className="flex items-start gap-3 border-t border-rule bg-surface-soft px-5 py-5 text-xs leading-5 text-ink-muted sm:px-7">
          ...
          >
            ...
            <div className="space-y-4 px-5 py-6 sm:px-7">
              ...
              >
                ...
                {submitting
                  ? "Starting privacy transform…"
                  : "Start privacy transform"}
              </Button>
            </div>
          </section>
        </form>
        ...
        if (mounted && (loadError as Error).name !== "AbortError")
          setError(
            loadError instanceof Error
              ? loadError.message
              : "This privacy job could not be loaded.",
          );
      }
    };
    void load();
    ...
    try {
      ...
      setError(
        acknowledgeError instanceof Error
          ...
          : "The quality acknowledgement could not be recorded.",
      );
    } finally {
    ...
    return (
      <div className="page-frame">
        <div
          ...
          aria-live="polite"
        >
          <span className="grid size-11 place-items-center rounded-lg bg-teal-soft text-teal-dark">
            <Icon name="spinner" className="size-5 animate-spin" />
          </span>
          <div>
            <h1 className="text-xl font-semibold text-ink">
              Loading protected job
            </h1>
            <p className="mt-1 text-sm text-ink-muted">
              Checking the privacy transform status…
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="page-frame">
      <div className="mx-auto max-w-5xl animate-enter-up">
        <button
          ...
          type="button"
          onClick={() => router.push("/video-privacy")}
        >
          <Icon name="back" className="size-4" />
          Start another privacy job
        </button>
        ...
        >
          <div className="border-b border-rule px-5 py-5 sm:px-7">
            ...
            <p className="mt-1 text-sm leading-6 text-ink-muted">
              Only privacy, output validation, and cleanup stages are shown
              here.
            </p>
          </div>
          <ol className="grid grid-cols-2 gap-px bg-rule sm:grid-cols-4">
            {job.stages.map((stage) => (
              <li key={stage.id} className="bg-surface px-4 py-5 sm:px-5">
        ...
        {job.status === "failed" ? (
          <div
            className="mt-6 flex items-start gap-3 rounded-lg border border-red/30 bg-red-soft px-4 py-4 text-sm text-red"
            role="alert"
          >
            <Icon name="alert" className="mt-0.5 size-5 shrink-0" />
            <div>
              <p className="font-bold">Privacy transform unavailable</p>
              <p className="mt-1 leading-6">
                {job.error ?? "The video could not be transformed safely."} No
                protected output is available.
              </p>
            </div>
          </div>
        ) : (
          complete && (
            <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(280px,0.85fr)]">
              ...
              >
                <div className="border-b border-rule px-5 py-5 sm:px-7">
                  <h2 id="preview-heading" className="text-base font-bold">
                    Protected preview frame
                  </h2>
                  <p className="mt-1 text-sm leading-6 text-ink-muted">
                    This frame is generated from the transformed output. The
                    original is never previewed here.
                  </p>
                </div>
                <div className="bg-surface-soft p-5 sm:p-7">
                  {job.previewUrl ? (
                    <img
                    ...
                    <div className="grid aspect-video place-items-center rounded-lg border border-rule bg-surface-muted text-center">
                      <div>
                        <Icon
                          name="shield"
                          className="mx-auto size-9 text-teal"
                        />
                        <p className="mt-3 text-sm font-bold text-ink">
                          Protected preview frame
                        </p>
                        <p className="mt-1 text-xs text-ink-muted">
                          Preview is available from the transformed artifact.
                        </p>
                      </div>
                    </div>
                  )}
              ...
              <section className="space-y-6">
                <div className="panel overflow-hidden">
                  <div className="border-b border-rule px-5 py-5">
                    <h2 className="text-base font-bold">Output policy</h2>
                    <p className="mt-1 text-sm leading-6 text-ink-muted">
                ...
                {job.requiresAcknowledgement && (
                  <div className="rounded-lg border border-amber/40 bg-amber-soft px-5 py-5">
                    <h2 className="text-sm font-bold text-ink">
                      Privacy quality needs review
                    </h2>
                    <p className="mt-2 text-sm leading-6 text-ink-muted">
                      The transform is usable, but detection was intermittent.
                      Review the protected preview and acknowledge this caveat
                      before downloading.
                    </p>
                    <Button
                      className="mt-4 h-auto border-0 bg-amber text-white hover:bg-amber/90"
                      type="button"
                    ...
                    >
                      {acknowledging
                        ...
                        : "Acknowledge and enable download"}
                    </Button>
                  </div>
                )}
                {job.qualityFlags.length > 0 && (
                  <div className="rounded-lg border border-rule bg-surface px-5 py-5">
                    <h2 className="text-sm font-bold text-ink">
                      Quality notes
                    </h2>
                    <ul className="mt-2 space-y-1 text-sm leading-6 text-ink-muted">
                      {job.qualityFlags.map((flag) => (
                        <li key={flag}>• {formatQualityFlag(flag)}</li>
                      ))}
                    </ul>
                  </div>
                )}
        ...
        <div className="mt-6 flex items-start gap-3 rounded-lg border border-rule bg-surface-soft px-4 py-4 text-xs leading-5 text-ink-muted">
          <Icon name="info" className="mt-0.5 size-4 shrink-0 text-amber" />
          <p>
            <span className="font-semibold text-ink">
              Research privacy transform.
            </span>{" "}
            The output is not guaranteed anonymous. Use only with approved,
            consented data and review the protected output before reuse.
          </p>
        </div>
      </div>
    </div>
...
function PolicyRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-4 border-b border-rule pb-3 last:border-0 last:pb-0">
      <span className="text-ink-muted">{label}</span>
      <span className="text-right font-semibold text-ink">{value}</span>
    </div>
  );
}
function StatusPill({ status }: { status: VideoPrivacyJob["status"] }) {
  const tone =
    status === "ready"
      ...
      : status === "failed" || status === "expired"
        ...
        : status === "needs_review"
          ...
          : "border-rule-strong bg-surface-soft text-ink-muted";
  return (
    <span
      className={`inline-flex min-h-8 items-center rounded-full border px-3 text-xs font-bold ${tone}`}
    >
      {statusLabel(status)}
    </span>
  );
}
function statusLabel(status: VideoPrivacyJob["status"]): string {
  return {
    queued: "Queued",
    preflight: "Preflight",
...
function formatQualityFlag(flag: string): string {
  return flag === "no_detection"
    ? "No detection was recorded; the transform failed closed."
    : flag === "intermittent_detection"
      ? "Detection was intermittent across the video."
      : flag.replaceAll("_", " ");
}
function formatExpiry(value: string | null): string {
  if (!value) return "Policy unavailable";
...
function UnavailableState({ message }: { message: string }) {
  return (
    <div className="page-frame">
      <div
        className="mx-auto max-w-2xl rounded-xl border border-red/30 bg-red-soft px-5 py-6"
        role="alert"
      >
        <span className="grid size-10 place-items-center rounded-lg bg-surface text-red">
          <Icon name="alert" className="size-5" />
        </span>
        <h1 className="mt-4 text-2xl font-semibold tracking-[-0.02em] text-ink">
          Protected job unavailable
        </h1>
        <p className="mt-3 text-sm leading-6 text-red">{message}</p>
        <Button asChild variant="outline" className="mt-6">
          <Link href="/video-privacy">Return to video privacy</Link>
        </Button>
      </div>
    </div>
  );
