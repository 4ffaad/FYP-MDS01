    ...
    <div className="page-frame">
      <div className="mx-auto max-w-4xl animate-enter-up">
        <div className="max-w-3xl">
          <p className="eyebrow">Standalone privacy workflow</p>
          <h1 className="mt-3 text-[clamp(2rem,5vw,3.4rem)] font-semibold leading-[1.05] tracking-[-0.055em] text-ink">Protect a patient video before review.</h1>
        ...
        <form className="mt-10 grid items-start gap-6 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]" onSubmit={(event) => void handleSubmit(event)}>
          ...
          <section className="panel overflow-hidden" aria-labelledby="profile-heading">
            ...
            <div className="space-y-3 px-5 py-6 sm:px-7" role="radiogroup" aria-labelledby="profile-heading">
              ...
              <Button className="button-primary mt-2 h-auto w-full border-0" type="submit" disabled={submitting || !file}>{submitting ? <Icon name="spinner" className="size-4 animate-spin" /> : <Icon name="arrow" className="size-4" />}{submitting ? "Starting privacy transform…" : "Start privacy transform"}</Button>
            </div>
          </section>
        </form>
      </div>
...
export function VideoPrivacyJobScreen({ jobId }: { jobId: string }) {
  ...
  if (!job) return <div className="page-frame"><div className="mx-auto max-w-3xl" aria-live="polite"><p className="eyebrow">Video privacy</p><h1 className="mt-3 text-3xl font-semibold tracking-tight text-ink">Loading protected job…</h1></div></div>;
  ...
  return <div className="page-frame">
    <div className="mx-auto max-w-5xl animate-enter-up">
      ...
      <div className="mt-6 flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">Protected video job</p><h1 className="mt-3 text-[clamp(2rem,5vw,3.4rem)] font-semibold leading-[1.05] tracking-[-0.055em] text-ink">{job.label}</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-ink-muted">{job.profileLabel} · {statusLabel(job.status)}</p></div><StatusPill status={job.status} /></div>
...
function UnavailableState({ message }: { message: string }) { return <div className="page-frame"><div className="mx-auto max-w-2xl"><p className="eyebrow">Video privacy</p><h1 className="mt-3 text-3xl font-semibold tracking-tight text-ink">Protected job unavailable</h1><p className="mt-4 text-sm leading-6 text-ink-muted">{message}</p><Button asChild variant="outline" className="mt-6"><Link href="/video-privacy">Return to video privacy</Link></Button></div></div>; }
