...
export function PrivacyPreview({ method, compact = false }: { method: PrivacyMethod; compact?: boolean }) {
  ...
  const afterPaths = useMemo(
    () => PRIVACY_SIGNAL_CHANNELS.map((_, index) => syntheticTrace(index, method.id === "signal-obfuscation")),
    [method.id],
  );
  const obfuscated = method.id === "signal-obfuscation";

  return (
    <section className="panel overflow-hidden" aria-labelledby="preview-heading">
      <div className={`${compact ? "px-4 py-4" : "px-5 py-5 sm:px-7"} border-b border-rule`}>
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 id="preview-heading" className={`${compact ? "text-sm" : "text-base"} font-bold`}>{compact ? "Privacy representation preview" : method.previewTitle ?? method.label}</h2>
            <p className="mt-1 text-sm leading-6 text-ink-muted">Illustrative preview — synthetic data, not your uploaded EEG.</p>
          </div>
      ...
      <div className={compact ? "px-4 py-4" : "px-5 py-6 sm:px-7"}>
        <div className="rounded-lg border border-rule bg-[#fbfcfe] p-4">
          ...
          <div className="mt-4 grid grid-cols-2 text-xs font-semibold">
            <span className="border-b-2 border-[#6b7b99] pb-2 text-[#52627e]">Before · staged</span>
            <span className="border-b-2 border-teal bg-teal-soft px-2 pb-2 text-right text-teal-dark">After · {obfuscated ? "obfuscated" : "metadata scrubbed"}</span>
          </div>

          <div className="mt-3 overflow-x-auto">
            <div className={`relative ${compact ? "min-w-[30rem]" : "min-w-[34rem]"}`} style={{ height: `${CHART_HEIGHT}px` }}>
              <svg
              ...
              >
                <defs>
                  <clipPath id={`${clipId}-before`} data-layer-clip="before">
                    <rect x="0" y="0" width={dividerPosition} height={CHART_HEIGHT} />
                  </clipPath>
                  <clipPath id={`${clipId}-after`} data-layer-clip="after">
                    <rect x={dividerPosition} y="0" width={afterReveal} height={CHART_HEIGHT} />
                  </clipPath>
                </defs>
                {PRIVACY_SIGNAL_CHANNELS.map((label, index) => {
                  const baseline = index * ROW_HEIGHT + ROW_HEIGHT / 2;
                  return <line key={label} x1="17" x2="100" y1={baseline} y2={baseline} stroke="#e5e7eb" strokeWidth="0.35" vectorEffect="non-scaling-stroke" />;
                })}
                <g clipPath={`url(#${clipId}-before)`} data-waveform-layer="before">
                  {beforePaths.map((path, index) => (
                    <path key={PRIVACY_SIGNAL_CHANNELS[index]} d={path} fill="none" stroke="#6b7b99" strokeWidth="0.75" vectorEffect="non-scaling-stroke" />
                  ))}
                </g>
                <g clipPath={`url(#${clipId}-after)`} data-waveform-layer="after">
                  <rect x={dividerPosition} y="0" width={afterReveal} height={CHART_HEIGHT} fill="#e6f5f2" opacity="0.7" />
                  {afterPaths.map((path, index) => (
                    <path key={PRIVACY_SIGNAL_CHANNELS[index]} d={path} fill="none" stroke="#087f75" strokeWidth="0.9" vectorEffect="non-scaling-stroke" />
                  ))}
                </g>
                {afterReveal > 0 && afterReveal < 100 && (
                  <line x1={dividerPosition} x2={dividerPosition} y1="0" y2={CHART_HEIGHT} stroke="#087f75" strokeWidth="1.35" vectorEffect="non-scaling-stroke" />
                )}
              </svg>
              <div className="pointer-events-none absolute inset-y-0 left-0 flex w-[17%] flex-col justify-around bg-[#fbfcfe] pr-2 font-mono text-[10px] font-semibold text-ink-muted">
                {PRIVACY_SIGNAL_CHANNELS.map((label) => <span key={label}>{label}</span>)}
              </div>
            </div>
          </div>
          ...
          <div className="mt-5 rounded-lg border border-rule bg-surface px-3.5 py-3">
            <div className="flex items-center justify-between gap-3 text-xs font-semibold text-ink">
              ...
              <span className="font-mono tabular-nums text-teal-dark">{100 - afterReveal}% before · {afterReveal}% after</span>
            </div>
            <Slider
            ...
            />
            <div className="mt-2 flex justify-between text-[0.68rem] text-ink-faint"><span>100% before</span><span>50 / 50</span><span>100% after</span></div>
          </div>
        </div>

        <p className="mt-5 text-sm leading-6 text-ink-muted">{method.previewDescription}</p>
      ...
      <div className="border-t border-rule bg-amber-soft px-5 py-4 text-xs leading-5 text-amber">
        <span className="font-bold">Research boundary.</span> This is a synthetic illustration. Signal obfuscation reduces detail experimentally; it is not a formal anonymity guarantee.
      </div>
    </section>
  );
}
