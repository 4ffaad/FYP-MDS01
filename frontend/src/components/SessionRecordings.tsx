...
export function SessionRecordings({ session }: { session: Session }) {
  const processedRecordings = session.recordings.filter((recording) => recording.status === "inferred" || recording.status === "failed");
  const completedRecordings = processedRecordings.filter((recording) => recording.status === "inferred");
  ...
  const visibleRecordings = useMemo(() => {
    ...
    if (activeFilter === "review") return processedRecordings.filter((recording) => recording.status === "failed");
 ...
 return (
   <div>
      <div className="border-b border-rule bg-surface">
        <Tabs value={activeFilter} onValueChange={(value) => { if (value === "alerts" || value === "review" || value === "all") setFilter(value); }} className="!block">
          <TabsList variant="line" className="w-full max-w-full justify-start gap-1 overflow-x-auto rounded-none px-4 py-3 sm:px-5">
            <TabsTrigger value="alerts" className="h-9 flex-none rounded-full px-3 text-xs font-semibold data-[state=active]:bg-teal-soft data-[state=active]:text-teal-dark">{alertLabel} <span className="font-mono tabular-nums">{alertCount}</span></TabsTrigger>
            <TabsTrigger value="review" className="h-9 flex-none rounded-full px-3 text-xs font-semibold data-[state=active]:bg-amber-soft data-[state=active]:text-amber">Needs review <span className="font-mono tabular-nums">{processedRecordings.filter((recording) => recording.status === "failed").length}</span></TabsTrigger>
            <TabsTrigger value="all" className="h-9 flex-none rounded-full px-3 text-xs font-semibold data-[state=active]:bg-surface-muted">All processed <span className="font-mono tabular-nums">{processedRecordings.length}</span></TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
      {pendingCount > 0 && <div className="min-h-11 border-b border-rule bg-surface-soft px-4 py-3 text-xs text-ink-muted sm:px-5" aria-live="polite"><Icon name="spinner" className="mr-1.5 inline size-3.5 animate-spin text-teal" />{pendingCount} recording{pendingCount === 1 ? "" : "s"} still processing. Only completed results are shown.</div>}
      {visibleRecordings.length > 0 ? <div className="divide-y divide-rule">{visibleRecordings.map((recording) => <RecordingRow key={recording.recordId} recording={recording} total={session.recordings.length} />)}</div> : <div className="px-5 py-10 text-center sm:px-7"><p className="text-sm font-bold text-ink">{activeFilter === "alerts" ? `No ${development ? "development flags" : "model alerts"} detected.` : activeFilter === "review" ? "No recordings need review." : "No processed recordings yet."}</p><p className="mt-2 text-xs leading-5 text-ink-muted">{activeFilter === "alerts" ? "Use All processed to inspect recordings that did not cross the model threshold." : "The list will update as processing continues."}</p></div>}
   </div>
 );
}
...
function RecordingRow({ recording, total }: { recording: Recording; total: number }) {
 ...
 const alertClass = development || !calibrated ? "border-l-4 border-amber bg-amber-soft/75 text-amber" : "border-l-4 border-red bg-red-soft/60 text-red";
  const content = (
    <>
      <div className="flex min-w-0 items-start gap-3">
        <span className={`grid size-8 shrink-0 place-items-center rounded-md ${hasModelAlert ? development || !calibrated ? "bg-amber text-white" : "bg-red text-white" : "bg-surface-muted text-teal"}`} aria-hidden="true">
  ...
  return <Link className={`${rowClass} transition-colors ${hasModelAlert ? development || !calibrated ? "hover:bg-amber-soft" : "hover:bg-red-soft" : "hover:bg-teal-soft/20"}`} href={`/results/${encodeURIComponent(recording.recordId)}`} aria-label={`Open ${label} results`}>{content}</Link>;
}
...
export function SessionGroup({ session, onDelete }: { session: Session; onDelete?: (sessionId: string) => Promise<void> }) {
  ...
  const alertCount = completedRecordings.filter((recording) => recording.modelAlertWindowCount > 0).length;
  const development = completedRecordings.some((recording) => recording.scoreType === "development_score");
  const calibrated = completedRecordings.some((recording) => recording.scoreType === "calibrated_probability");
  const alertLabel = development ? "development flag" : calibrated ? "model alert" : "research flag";
  const alertToneClass = development || !calibrated ? "font-semibold text-amber" : "font-semibold text-red";
      ...
      <section className={`panel relative overflow-hidden transition-transform duration-200 ${swiped ? "-translate-x-24" : ""}`} aria-labelledby={`session-${session.sessionId}`}>
        <Link className="group block bg-surface-soft px-4 py-5 transition-colors hover:bg-surface sm:px-5 sm:py-6" href={`/sessions/${encodeURIComponent(session.sessionId)}`} aria-label={`Open session ${session.sessionId}`}>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <p className="text-xs font-semibold uppercase tracking-[0.1em] text-ink-muted">Analysis session</p>
            ...
            <div className="flex items-center gap-3 sm:pt-1">
              ...
              <Icon name="chevron" className="size-4 text-ink-muted transition-transform group-hover:translate-x-0.5" />
            </div>
          </div>
          <div className="mt-5 grid gap-4 border-t border-rule pt-4 sm:grid-cols-[minmax(0,1fr)_minmax(15rem,0.8fr)] sm:gap-8">
            <div>
              ...
              <Progress value={session.progress.percent} className="mt-2 h-2 bg-surface-muted [&_[data-slot=progress-indicator]]:rounded-full [&_[data-slot=progress-indicator]]:bg-teal" aria-label={`Processing progress for ${session.sessionId}`} />
            ...
            <div className="text-xs leading-5 sm:border-l sm:border-rule sm:pl-6">
              ...
              <p className={`mt-1 ${analysisReady && alertCount > 0 ? alertToneClass : "text-ink-muted"}`}>{modelSummary}</p>
              {analysisReady && alertCount > 0 && <p className="mt-2 text-xs text-ink-muted">Flagged times: {alertSummary.values.join(", ")}{alertSummary.remaining > 0 ? ` +${alertSummary.remaining} more intervals` : ""}</p>}
            </div>
          </div>
        </Link>
        {onDelete && <div className="flex items-center justify-between gap-4 border-t border-rule px-4 py-2.5 sm:px-5"><span className="text-xs text-ink-muted">Open the session to review its recordings.</span><DeleteSessionButton session={session} onDelete={onDelete} /></div>}
      </section>
    </div>
  );
}
...
function ModelLabel({ recording }: { recording: Recording }) {
  ...
  const modelLabel = recording.status !== "inferred"
    ...
    : recording.modelAlertWindowCount > 0
      ...
      : "No flagged windows";
  const modelClass = recording.status !== "inferred" ? "text-ink-muted" : recording.modelAlertWindowCount > 0 ? development || !calibrated ? "font-semibold text-amber" : "font-semibold text-red" : "text-ink-muted";
  return <><p className={`mt-1 text-xs ${modelClass}`}>{modelLabel}</p>{recording.status === "inferred" && recording.modelAlertWindowCount > 0 && recording.alertIntervals.length > 0 && <p className="mt-1 text-xs text-ink-muted">First flagged interval: {formatInterval(recording.alertIntervals[0].startSeconds, recording.alertIntervals[0].endSeconds)}{recording.alertIntervals.length > 1 ? ` +${recording.alertIntervals.length - 1} more` : ""}</p>}</>;
}

/** Return compact model-alert times and a count of omitted intervals. */
function sessionAlertSummary(session: Session): { values: string[]; remaining: number } {
  const values = session.recordings
    .filter((recording) => recording.status === "inferred" && recording.modelAlertWindowCount > 0)
    .flatMap((recording) => {
      const interval = recording.alertIntervals[0];
      return interval ? [`Recording ${String(recording.sequenceIndex).padStart(2, "0")} · ${formatInterval(interval.startSeconds, interval.endSeconds)}`] : [];
    });
  return { values: values.slice(0, 3), remaining: Math.max(0, values.length - 3) };
}
...
export function DeleteSessionButton({ session, onDelete, variant = "inline" }: { session: Session; onDelete: (sessionId: string) => Promise<void>; variant?: "inline" | "swipe" }) {
