...
export function SignalViewer({
  ...
  const initialStart = clamp(
    (firstAlert?.startSeconds ?? browseStart) - 2,
    browseStart,
    maxStart,
  );
  ...
  const [loading, setLoading] = useState(
    ENABLE_SIGNAL_PREVIEW && Boolean(firstAlert),
  );
  ...
  const representation =
    preview?.representation === "signal-obfuscated"
      ? "Signal obfuscated"
      : "Metadata scrubbed";
  ...
  return (
    <section className="panel overflow-hidden" aria-labelledby="signal-heading">
      <SignalHeader
        fullPreview={ENABLE_FULL_SIGNAL_PREVIEW}
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0 flex-1">
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              <p className="text-sm font-semibold text-ink">
                Viewing {formatTime(sliderStart)}–{formatTime(currentEnd)}
              </p>
              <p className="font-mono text-xs tabular-nums text-ink-muted">
                {formatTime(browseStart)} <span aria-hidden="true">—</span>{" "}
                {formatTime(browseEnd)}
              </p>
            </div>
            <div className="relative mt-4 px-1">
              {alertWindows
                .filter(
                  (window) =>
                    window.endSeconds > browseStart &&
                    window.startSeconds < browseEnd,
                )
                .map((window, index) => {
                  const range = Math.max(1, browseEnd - browseStart);
                  const left =
                    ((window.startSeconds - browseStart) / range) * 100;
                  return (
                    <span
                      key={`${window.startSeconds}-${index}`}
                      className="pointer-events-none absolute top-1/2 z-10 h-3 w-0.5 -translate-y-1/2 bg-amber"
            ...
            <p className="mt-2 text-xs text-ink-muted">
              Drag to move through the recording. Amber ticks mark windows that
              crossed the model threshold.
            </p>
          </div>
          ...
          <div className="flex shrink-0 flex-wrap gap-2">
            <Button
              variant="outline"
              size="lg"
              className="border-rule-strong bg-surface"
              disabled={sliderStart <= browseStart || loading}
              onClick={() => moveTo(sliderStart - viewportDuration)}
            >
            ...
            <Button
              ...
              size="lg"
              className="border-rule-strong bg-surface"
              disabled={sliderStart >= maxStart || loading}
              onClick={() => moveTo(sliderStart + viewportDuration)}
            >
      ...
      <div className="px-4 py-5 sm:px-7 sm:py-7">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-ink-muted">
          ...
          <span className="inline-flex items-center gap-2">
            <span
              className="h-3 w-3 rounded-sm bg-amber/70"
              aria-hidden="true"
            />
            Threshold crossing
          </span>
        ...
        <p className="mt-3 text-xs leading-5 text-ink-muted">
          Each channel is normalized for display only. The stored signal and
          model input are not changed by this viewer.
        </p>
      </div>
    </section>
  );
    ...
    <div className="flex flex-col gap-3 px-5 py-5 sm:flex-row sm:items-start sm:justify-between sm:px-7">
      <div>
        ...
        <p className="mt-1 max-w-3xl text-sm leading-6 text-ink-muted">
          ...
          {fullPreview
            ? "The complete retained recording is available in this local-development view."
            : "Only retained context around the first flagged window is available."}
        </p>
      </div>
      <span className="w-fit rounded-full border border-rule bg-surface-soft px-3 py-1 text-xs font-semibold text-ink-muted">
        {representation}
      </span>
    </div>
  );
...
function SignalNotice({ children }: { children: string }) {
  return (
    <section
      className="rounded-lg border border-rule bg-surface-soft px-5 py-5 sm:px-7"
...
function SignalCanvas({ preview }: { preview: SignalPreview }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      preview.channels.forEach((channel, channelIndex) => {
        ...
        const mean =
          channelIndices.reduce(
            (sum, index) => sum + channel.samples[index],
            0,
          ) / Math.max(1, channelIndices.length);
        const variance =
          channelIndices.reduce(
  ...
  const intervalText = preview.flaggedIntervals
    .map(
      (interval) =>
        `${formatTime(interval.startSeconds)}–${formatTime(interval.endSeconds)}`,
    )
    .join(", ");
  return (
    <div>
