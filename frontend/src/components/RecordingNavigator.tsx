...
export function RecordingNavigator({
  session,
  activeRecordId,
}: {
  session: Session;
  ...
  const readyCount = session.recordings.filter(
    (recording) => recording.status === "inferred",
  ).length;
  const failedCount = session.recordings.filter(
    (recording) => recording.status === "failed",
  ).length;
  const active = session.recordings.find(
    (recording) => recording.recordId === activeRecordId,
  );
  ...
  if (session.recordings.length === 1) {
    ...
    const alertTone = calibrated
      ? "bg-red-soft text-red"
      : "bg-amber-soft text-amber";
    return (
      <nav
        className="panel flex items-center gap-3 px-5 py-4 sm:px-7"
        aria-label="Recordings in this session"
      >
        <span
          className={`grid size-9 shrink-0 place-items-center rounded-lg ${hasAlert ? alertTone : "bg-teal-soft text-teal-dark"}`}
        >
          <Icon name={hasAlert ? "alert" : "check"} className="size-4" />
        </span>
        <span>
          ...
          <span className="mt-0.5 block text-xs text-ink-muted">
            {hasAlert
              ? `${active?.modelAlertWindowCount} flagged windows`
              : "No flagged windows"}
          </span>
        </span>
      </nav>
      ...
      <details open={session.recordings.length <= 6}>
        <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 marker:hidden sm:px-7 [&::-webkit-details-marker]:hidden">
          <span className="flex min-w-0 items-center gap-3">
            <span className="grid size-9 shrink-0 place-items-center rounded-lg bg-teal-soft text-teal-dark">
              <Icon name="list" className="size-4" />
            </span>
            <span className="min-w-0">
              <span className="block text-sm font-bold text-ink">
                Recording {String(activeNumber).padStart(2, "0")} of{" "}
                {session.recordings.length}
              </span>
              <span className="mt-0.5 block truncate text-xs text-ink-muted">
                {readyCount} ready
                {failedCount > 0 ? ` · ${failedCount} need review` : ""}
              </span>
            </span>
          </span>
          <span className="inline-flex shrink-0 items-center gap-2 text-xs font-semibold text-teal-dark">
            Change recording{" "}
            <Icon
              name="chevron"
              className="size-4 transition-transform in-open:rotate-90"
            />
          </span>
        </summary>
        <div className="max-h-80 divide-y divide-rule overflow-y-auto border-t border-rule bg-surface-soft">
          {session.recordings.map((recording) => (
            <RecordingLink
              ...
              active={recording.recordId === activeRecordId}
            />
          ))}
        </div>
      </details>
...
function RecordingLink({
  ...
  const content = (
    <>
      <span
        className={`grid size-8 shrink-0 place-items-center rounded-lg ${tone}`}
        aria-hidden="true"
      >
        <Icon
          name={
            recording.status === "failed" || hasModelAlert
              ? "alert"
              : recording.status === "inferred"
                ? "check"
                : "file"
      ...
      <span className="min-w-0 flex-1">
        ...
        <span
          className={`mt-1 block text-xs leading-5 ${hasModelAlert ? (development || !calibrated ? "font-semibold text-amber" : "font-semibold text-red") : "text-ink-muted"}`}
        >
          {summary}
        </span>
        {recording.errorMessage && (
          <span className="mt-1 block text-xs leading-5 text-red">
            {recording.errorMessage}
          </span>
        )}
      </span>
