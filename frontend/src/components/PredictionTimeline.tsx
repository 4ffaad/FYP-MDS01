...
export function PredictionTimeline({ predictions, durationSeconds, threshold = 0.5, scoreType = "development_score" }: { predictions: PredictionWindow[]; durationSeconds: number; threshold?: number; scoreType?: string }) {
  ...
  const recordingDuration = Math.max(durationSeconds, predictions.at(-1)?.endSeconds ?? 1, 1);
  const linePredictions = useMemo(() => linePoints(predictions, MAX_LINE_POINTS), [predictions]);
  const alertPredictions = useMemo<IndexedPrediction[]>(
    () => predictions.flatMap((prediction, windowIndex) => prediction.seizureDetected ? [{ prediction, windowIndex }] : []),
    [predictions],
  );
  const alertIndexByWindow = useMemo(
    () => new Map(alertPredictions.map((item, alertIndex) => [item.windowIndex, alertIndex])),
    [alertPredictions],
  );
  const hoverPredictions = useMemo<IndexedPrediction[]>(() => {
    ...
    return predictions.flatMap((prediction, windowIndex) => (
      prediction.seizureDetected || windowIndex % stride === 0 || windowIndex === predictions.length - 1
  ...
  const handleAlertNavigation = (event: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (event.key === "ArrowRight" || event.key === "ArrowDown") nextIndex = Math.min(alertPredictions.length - 1, selectedAlertIndex + 1);
    else if (event.key === "ArrowLeft" || event.key === "ArrowUp") nextIndex = Math.max(0, selectedAlertIndex - 1);
    ...
    <div aria-label="Prediction score timeline" role="group">
      <div className="overflow-x-auto rounded-lg border border-rule bg-surface-soft p-2 sm:p-3">
        <svg className="h-auto min-w-[680px] w-full" viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`} role="img" aria-labelledby="prediction-chart-title prediction-chart-description">
          ...
          {[1, 0.5, 0].map((value) => (
            <g key={value}>
              <line x1={PADDING.left} y1={y(value)} x2={CHART_WIDTH - PADDING.right} y2={y(value)} stroke="currentColor" strokeWidth="1" className="text-rule" />
            ...
            return (
              <g key={`point-${windowIndex}`}>
                <circle
                  ...
                  onMouseEnter={alertIndex >= 0 ? () => setActiveAlertIndex(alertIndex) : undefined}
                  onFocus={alertIndex >= 0 ? () => setActiveAlertIndex(alertIndex) : undefined}
                >
                  <title>{label}</title>
                </circle>
          ...
          <line x1={PADDING.left} y1={thresholdY} x2={CHART_WIDTH - PADDING.right} y2={thresholdY} stroke="currentColor" strokeDasharray="5 5" className="text-amber" />
          <rect x={CHART_WIDTH - PADDING.right - 128} y={thresholdY - 20} width="128" height="18" rx="4" className="fill-surface" />
          <text x={CHART_WIDTH - PADDING.right - 8} y={thresholdY - 7} textAnchor="end" className="fill-amber text-[11px] font-semibold">Alert threshold · {threshold.toFixed(2)}</text>
          <text x={PADDING.left} y={CHART_HEIGHT - 10} className="fill-ink-muted text-[11px]">0:00</text>
          <text x={PADDING.left + plotWidth / 2} y={CHART_HEIGHT - 10} textAnchor="middle" className="fill-ink-muted text-[11px]">{formatSeconds(recordingDuration / 2)}</text>
          <text x={CHART_WIDTH - PADDING.right} y={CHART_HEIGHT - 10} textAnchor="end" className="fill-ink-muted text-[11px]">{formatSeconds(recordingDuration)}</text>
        </svg>
      </div>
...
function linePoints(predictions: PredictionWindow[], maxPoints: number): PredictionWindow[] {
  if (predictions.length <= maxPoints) return predictions;
  const alertCount = predictions.filter((prediction) => prediction.seizureDetected).length;
  if (alertCount >= maxPoints) return predictions.filter((prediction) => prediction.seizureDetected);
  const normalBudget = Math.max(2, maxPoints - alertCount);
  const stride = Math.max(1, Math.ceil(predictions.length / normalBudget));
  return predictions.filter((prediction, index) => prediction.seizureDetected || index === 0 || index === predictions.length - 1 || index % stride === 0);
}
