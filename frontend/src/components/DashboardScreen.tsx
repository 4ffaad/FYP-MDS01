    ...
    try {
      ...
      setError(
        refreshError instanceof Error
          ...
          : "EEG analyses could not be loaded.",
      );
    } finally {
  ...
  return (
    <div className="page-frame">
      <div className="animate-enter-up">
        <header className="flex flex-col justify-between gap-6 border-b border-rule pb-7 sm:flex-row sm:items-end">
          <div className="max-w-2xl">
            <h1
              ...
              className="text-[clamp(2rem,4vw,2.5rem)] font-semibold leading-[1.08] tracking-[-0.035em] text-ink"
            >
              EEG analysis
            </h1>
            <p className="mt-3 max-w-xl text-[0.94rem] leading-6 text-ink-muted">
              Review recordings, follow processing, and open flagged EEG
              windows.
            </p>
          </div>
        ...
        <div className="mt-5 flex flex-col justify-between gap-4 border-b border-rule py-4 sm:flex-row sm:items-center">
          <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-sm text-ink-muted">
            <span>
              ...
              EEG {sessions.length === 1 ? "session" : "sessions"}
            </span>
        ...
        {error && (
          <div
            className="mt-6 flex items-start justify-between gap-4 rounded-lg border border-red/30 bg-red-soft px-4 py-3 text-sm text-red"
            role="alert"
          >
...
function LoadingRows() {
  return (
    <div className="mt-6 space-y-3" aria-label="Loading sessions">
      <div className="h-28 animate-pulse rounded-lg border border-rule bg-surface" />
      <div className="h-28 animate-pulse rounded-lg border border-rule bg-surface" />
    </div>
  );
}
    ...
    <div className="py-16 sm:py-24">
      ...
      <p className="mt-2 max-w-md text-sm leading-6 text-ink-muted">
        {filtered
          ...
          : "Use New EEG analysis above to upload an archive and create a session."}
      </p>
    </div>
  );
}
